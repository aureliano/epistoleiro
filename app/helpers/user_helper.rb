  ...
  class App
    module UserHelper
      ...
      def build_user_account_creation_model(hash)
        ...
        user.home_page = hash[:home_page] unless hash[:home_page].to_s.empty?
      ...
      def validate_user_account_creation(hash)
        ...
        messages << I18n.translate('model.user.validation.home_page_length') if !hash[:home_page].to_s.empty? && (hash[:home_page].size < 5 || hash[:home_page].size > 100)
